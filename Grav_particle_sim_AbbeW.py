...
# Change line 312 to in ani.save() before you run this code to save the video it generates
...
def run_particle_sim(particle_mass_max = 50, frame_duration = 150, t_final = 500, 
L = 1000, num_bins = 100, softening_length = 25, Np = 100, Nt = 100, Nd = 2, v_max = 0.10, 
n = 12, dist_type = 't'):
    """
    ...
    k_max = 2*np.pi/(softening_length) # length scale info. can be resolved on / maximum frequency
    ...
    plt.subplots_adjust(left=0.05, bottom=0.05, right=0.95, top=0.95,wspace=0.15,hspace=0.4)
    ...
    def Pk(k, x, corr_x):               
        pk = 2*np.pi*pow(x,2)*corr_x*(np.sinc(k*x))  # pass just a single k value each time and array of xs, corrs
        return pk
    ...
    # save to mpeg file: 
    ani.save(r"C:\Users\abbew\PythonProjects\test.Mp4")
    print('Finished.')
    return 0
